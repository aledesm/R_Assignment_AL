...
Small error (easily fixed) near the beginning of the script and
few comments on the graphs, most of which say the same thing.


#### R-Assignment Alejandro Ledesma #####

```{r}
# COMMENT:  This is addressing the error in block of r code starting at line 127.
#           First you need to install and load packages into the environment. 
#           A person who pulls the script off the internet might not have the
#           same packages installed as you. I have pasted the lines I used in
#           my project as a demonstration.
...
View(fang_et_al_genotypes)  # In the future I would be careful putting a View() command
                            # directly in the script. I would make this a comment as an
                            # option for the person using your script. For me, I've found
                            # that if the file you are trying to view is large then it can
                            # temporarily freeze RStudio.
```
...
#   Error in add_rownames(maize_sorted, "SNP_ID") : 
#   could not find function "add_rownames"
#
# This was due to the 'dplyr' not being installed. I added a comment at 
# the beginning of the script addressing this issue.
#
# Also I received the following warning:
...
```{r}
library(dplyr) #COMMENT: This should be at the beginning of the script
...
```{r}
if (!require("ggplot2")) install.packages("ggplot2") #COMMENT: This should be at the beginning of the script
library(ggplot2)
...
```{r}
library(reshape2) #COMMENT: This should be at the beginning of the script
...
# COMMENT:  I don't see any missing sites (i.e. NA) on the graph.
#           Looking at the counts_combined file, the values of the 
#           total_na variable are all 0.
```
...
# COMMENT:  I don't see any missing sites (i.e. NA).
#           Looking at the counts_group_combined file, the values of the 
#           total_na variable are all 0.
```
...
# COMMENT: I don't see any missing sites (i.e. NA) on the graph
#           Looking at the maize_counts_group_combined file, the values of the 
#           total_na variable are all 0.
#
#     Also, if the goal is to compare the proportion of each category in each group
#     then you need to use a position adjustment in the geom_bar command of your 
#     ggplot statement. [Ex: Switch position = "stack" to position "fill"]
# ggplot(counts_group_combined,aes(x=Group,y=value,fill=variable))+geom_bar(stat="identity",position = "fill")
```
...
# Try to create a new variable or plot different data. This is showing the same
# thing as the plot above except these are points instead of bars.
```
